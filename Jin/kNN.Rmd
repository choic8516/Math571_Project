...
```
It looks like there are two picking areas, around 14 and around 36. They have close accuracy which are about `r round(opt_acc, digits = 4)*100`%.
...
```
The plot shows that we have two possible optimal k-value range. First one is around 3 and second one is around 160. Both of them have about `r round(opt_acc, digits = 4)*100`% accuracy roughly. It's just slightly better than tosing a coin.