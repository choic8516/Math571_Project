...
for (page in c(0:2)) {
        art <- fromJSON(paste0("http://api.nytimes.com/svc/search/v2/articlesearch.json?begin_date=",temp_begin_date,"&end_date=",temp_end_date,"&api-key=",cristobal_key,"&page=",page,"&fq=news_desk:(Financial)"), flatten = TRUE) %>%  data.frame()
        if (page == 0) {
...
WriteXLS::WriteXLS(headlines,"nyt_headlines2.xls")
```
