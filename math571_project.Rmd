...
scores <- read.xlsx("connor_data/nyt_headlines_2000_scored.xls", sheetIndex = 1,stringsAsFactors=FALSE)
sub_scores <- scores
...
kk <- lapply(ll,function(x) table(cut(t(x), breaks =  seq(-1, 1, by = .25))))

final <- data.frame(rbind(kk$X1,kk$X2))

for (i in (length(kk) - 2)) {
...
colnames(final) <- c("Very Negative", "Negative", "Moderately Negative", "Negative Neuter", "Positive Neuter", "Moderately Positive", "Postive", "Very Positive"， "Date")
