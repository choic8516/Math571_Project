...
adapted.df<-cbind(combined.df[3:Nrows,c(1,27)],combined.df[1:(Nrows-2),2:26],
                 combined.df[2:(Nrows-1),2:26])
...
adapted.df<-cbind(adapted.df[1:2], apply(adapted.df[,3:52],1,paste, collapse=" "),stringsAsFactors=F)
...
train.index<-c(1:4000)
test.index<-c(4001:5000)
...
#NB
nb.model<-textmodel_nb(train.dfm,labels.train)
